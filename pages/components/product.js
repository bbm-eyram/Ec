...
export default function Product({
  ...
  async function createProduct(ev) {
    ...
    const data = { title, description, price, images, category, details, brand, gender, sizes, colors };
    if (_id) {
    ...
    return<>
     <form onSubmit={createProduct} className="mx-auto my-4 max-w-screen-sm">
      ...
      {/* Spinner during upload */}
        ...
        {!isUploading && (
            <div className=" grid grid-cols-3 sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 mb-2">
              <ReactSortable list={Array.isArray(images) ? images : []} setList={updateImagesOrder} animation={200}  className="w-[350px] h-auto  gap-2 flex  justify-between align-items-center">
      ...
      <div className="items-center my-4">
          ...
          <button
            type="submit"
            className="rounded-lg border border-green-500 bg-primary-500 px-5 py-2.5 text-center text-sm font-medium text-black shadow-sm transition-all hover:border-primary-700 hover:bg-green-500 hover:text-white focus:ring focus:ring-primary-200 disabled:cursor-not-allowed disabled:border-primary-300 disabled:bg-primary-300 w-full"
          >
            Save Product
          </button>
          </div>
